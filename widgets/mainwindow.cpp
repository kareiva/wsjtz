  ...
  m_block_udp_status_updates {false}
{
  ...
  setWindowTitle (program_title () +  " (WSJT-Z MOD v1.33 by SQ9FVE)") ;
  ...
  ui->respondComboBox->addItem("CQ: Max Strength");
  ...
  labAz.setText("");
  tableInit();
  ui->tableBand->addAction(ui->actionIgnore_station);
  ui->tableBand->addAction(ui->actionCall_next);
  ui->tableBand->addAction(ui->actionClear);
  ui->tableBand->addAction(ui->actionSet_Rx_Freq);
  ui->tableBand->addAction(ui->actionQRZ_Lookup);
  ui->tableBand->addAction(ui->actionCopy);
  ui->tableRx->addAction(ui->actionIgnore_station);
  ui->tableRx->addAction(ui->actionCall_next);
  ui->tableRx->addAction(ui->actionClear);
  ui->tableRx->addAction(ui->actionSet_Rx_Freq);
  ui->tableRx->addAction(ui->actionQRZ_Lookup);
  ui->tableRx->addAction(ui->actionCopy);

  connect(ui->tableBand, SIGNAL(messageClicked(QString)), this, SLOT(tableBandItemClicked(QString)));
  connect(ui->tableRx, SIGNAL(messageClicked(QString)), this, SLOT(tableBandItemClicked(QString)));
  connect(ui->tableBand, SIGNAL(ctrlClicked(QString)), this, SLOT(ctrlClicked(QString)));
  connect(ui->tableRx, SIGNAL(ctrlClicked(QString)), this, SLOT(ctrlClicked(QString)));
  connect(ui->tableBand, SIGNAL(clicked(int)), this, SLOT(clicked(int)));
  connect(ui->tableRx, SIGNAL(clicked(int)), this, SLOT(clicked(int)));

  connect (&m_config, &Configuration::columns_changed, this, &MainWindow::tableInit);
  ...
  */
  ...
  if (!m_infoMessageShown) {
          ZMessage();
          m_infoMessageShown = true;
  }
    ...
    connect (&m_config, &Configuration::qrz_config_changed, this, &MainWindow::qrzInit);
    networkManager = new QNetworkAccessManager(this);

    connect (statusBar(), SIGNAL(clicked(bool)), this, SLOT(tx_watchdog(false)));

    // Initialize QRZ lookup
    if (m_config.qrzComUn().length() > 0 && m_config.qrzComPw().length() > 0) {
        qrzInit();
    }

    on_btn_clearIgnore_clicked();

    if ( m_config.special_op_id() == SpecOp::HOUND ) ui->cb_specialMode->setCurrentIndex(2);
    if ( m_config.special_op_id() == SpecOp::FOX ) ui->cb_specialMode->setCurrentIndex(1);
...
void MainWindow::initialize_fonts ()
{
  ...
  // Z
  if (m_unfilteredView) m_unfilteredView->setFont(m_config.decoded_text_font ());
  if (m_pskReporterView) m_pskReporterView->setFont(m_config.decoded_text_font ());
}
...
// Z
  m_settings->setValue("ignoreCQXX",ui->le_ignoreCQXX->text());
  m_settings->setValue("customAlerts",  ui->le_CustomAlerts->text());
  m_settings->setValue ("ignoreCQTargetIndex", ui->cb_ignoreCQTarget->currentIndex());
  ...
  m_settings->setValue ("filter_countryB4", ui->cb_countryB4->isChecked() );
  m_settings->setValue ("filter_countryB4onBand", ui->cb_countryB4onBand->isChecked() );
  m_settings->setValue ("filter_gridB4", ui->cb_gridB4->isChecked() );
  m_settings->setValue ("filter_gridB4onBand", ui->cb_gridB4onBand->isChecked() );
  m_settings->setValue ("filter_continentB4", ui->cb_continentB4->isChecked() );
  m_settings->setValue ("filter_continentB4onBand", ui->cb_continentB4onBand->isChecked() );
  m_settings->setValue ("filter_CQZoneB4", ui->cb_CQZoneB4->isChecked() );
  m_settings->setValue ("filter_CQZoneB4onBand", ui->cb_CQZoneB4onBand->isChecked() );
  m_settings->setValue ("filter_ITUZoneB4", ui->cb_ITUZoneB4->isChecked() );
  m_settings->setValue ("filter_ITUZoneB4onBand", ui->cb_ITUZoneB4onBand->isChecked() );
  m_settings->setValue ("alert_callB4", ui->cb_callB4_alert->isChecked() );
  m_settings->setValue ("alert_callB4onBand", ui->cb_callB4onBand_alert->isChecked() );
  m_settings->setValue ("alert_countryB4", ui->cb_countryB4_alert->isChecked() );
  m_settings->setValue ("alert_countryB4onBand", ui->cb_countryB4onBand_alert->isChecked() );
  m_settings->setValue ("alert_gridB4", ui->cb_gridB4_alert->isChecked() );
  m_settings->setValue ("alert_gridB4onBand", ui->cb_gridB4onBand_alert->isChecked() );
  m_settings->setValue ("alert_continentB4", ui->cb_continentB4_alert->isChecked() );
  m_settings->setValue ("alert_continentB4onBand", ui->cb_continentB4onBand_alert->isChecked() );
  m_settings->setValue ("alert_CQZoneB4", ui->cb_CQZoneB4_alert->isChecked() );
  m_settings->setValue ("alert_CQZoneB4onBand", ui->cb_CQZoneB4onBand_alert->isChecked() );
  m_settings->setValue ("alert_ITUZoneB4", ui->cb_ITUZoneB4_alert->isChecked() );
  m_settings->setValue ("alert_ITUZoneB4onBand", ui->cb_ITUZoneB4onBand_alert->isChecked() );
  m_settings->setValue ("filter_pte_prefixFilter", ui->pte_prefixFilter->toPlainText());
  m_settings->setValue ("filter_pte_stateFilter", ui->pte_stateFilter->toPlainText());
  m_settings->setValue ("filter_MindB", ui->sbMindB->value());
  m_settings->setValue ("filter_cb_prefixFilter", ui->cb_prefixFilter->currentIndex());
  m_settings->setValue ("filter_cb_stateFilter", ui->cb_stateFilter->currentIndex());
  m_settings->setValue ("filter_c_EU", ui->cb_c_EU->isChecked());
  m_settings->setValue ("filter_c_AF", ui->cb_c_AF->isChecked());
  m_settings->setValue ("filter_c_AN", ui->cb_c_AN->isChecked());
  m_settings->setValue ("filter_c_AS", ui->cb_c_AS->isChecked());
  m_settings->setValue ("filter_c_NA", ui->cb_c_NA->isChecked());
  m_settings->setValue ("filter_c_SA", ui->cb_c_SA->isChecked());
  ...
  if (m_unfilteredView && m_unfilteredView->isVisible ()) {
      m_settings->setValue ("rawViewGeometry", m_unfilteredView->saveGeometry() );
  }
  ...
  m_settings->setValue ("infoMessageShown", m_infoMessageShown);
...
void MainWindow::readSettings()
{
 ...
 // Z
  ...
  ui->cb_countryB4->setChecked(m_settings->value("filter_countryB4", false).toBool());
  ui->cb_countryB4onBand->setChecked(m_settings->value("filter_countryB4onBand", false).toBool());
  ui->cb_gridB4->setChecked(m_settings->value("filter_gridB4", false).toBool());
  ui->cb_gridB4onBand->setChecked(m_settings->value("filter_gridB4onBand", false).toBool());
  ui->cb_continentB4->setChecked(m_settings->value("filter_continentB4", false).toBool());
  ui->cb_continentB4onBand->setChecked(m_settings->value("filter_continentB4onBand", false).toBool());
  ui->cb_CQZoneB4->setChecked(m_settings->value("filter_CQZoneB4", false).toBool());
  ui->cb_CQZoneB4onBand->setChecked(m_settings->value("filter_CQZoneB4onBand", false).toBool());
  ui->cb_ITUZoneB4->setChecked(m_settings->value("filter_ITUZoneB4", false).toBool());
  ui->cb_ITUZoneB4onBand->setChecked(m_settings->value("filter_ITUZoneB4onBand", false).toBool());
  ui->cb_callB4_alert->setChecked(m_settings->value("alert_callB4", false).toBool());
  ui->cb_callB4onBand_alert->setChecked(m_settings->value("alert_callB4onBand", false).toBool());
  ui->cb_countryB4_alert->setChecked(m_settings->value("alert_countryB4", false).toBool());
  ui->cb_countryB4onBand_alert->setChecked(m_settings->value("alert_countryB4onBand", false).toBool());
  ui->cb_gridB4_alert->setChecked(m_settings->value("alert_gridB4", false).toBool());
  ui->cb_gridB4onBand_alert->setChecked(m_settings->value("alert_gridB4onBand", false).toBool());
  ui->cb_continentB4_alert->setChecked(m_settings->value("alert_continentB4", false).toBool());
  ui->cb_continentB4onBand_alert->setChecked(m_settings->value("alert_continentB4onBand", false).toBool());
  ui->cb_CQZoneB4_alert->setChecked(m_settings->value("alert_CQZoneB4", false).toBool());
  ui->cb_CQZoneB4onBand_alert->setChecked(m_settings->value("alert_CQZoneB4onBand", false).toBool());
  ui->cb_ITUZoneB4_alert->setChecked(m_settings->value("alert_ITUZoneB4", false).toBool());
  ui->cb_ITUZoneB4onBand_alert->setChecked(m_settings->value("alert_ITUZoneB4onBand", false).toBool());
  ui->pte_prefixFilter->setPlainText(m_settings->value("filter_pte_prefixFilter", "").toString());
  ui->pte_stateFilter->setPlainText(m_settings->value("filter_pte_stateFilter", "").toString());
  ui->sbMindB->setValue(m_settings->value("filter_MindB", -30).toInt());
  ui->cb_prefixFilter->setCurrentIndex(m_settings->value("filter_cb_prefixFilter", 0).toInt());
  ui->cb_stateFilter->setCurrentIndex(m_settings->value("filter_cb_stateFilter", 0).toInt());
  ui->cb_c_EU->setChecked(m_settings->value("filter_c_EU", true).toBool());
  ui->cb_c_AF->setChecked(m_settings->value("filter_c_AF", true).toBool());
  ui->cb_c_AN->setChecked(m_settings->value("filter_c_AN", true).toBool());
  ui->cb_c_AS->setChecked(m_settings->value("filter_c_AS", true).toBool());
  ...
  m_infoMessageShown = m_settings->value("infoMessageShown", false).toBool();
...
{
  // Z
  log("AutoTxMode: " + QString::number(state));
...
void MainWindow::createStatusBar()                           //createStatusBar
{
  ...
  // Z
  tx_status_label.setStyleSheet ("QLabel{color: #000000; background-color: #00aa00}");
  ...
  labDate.setMinimumSize (QSize {30, 18});
  labDate.setFrameStyle (QFrame::Panel | QFrame::Sunken);
  statusBar()->addWidget (&labDate);

  qso_count.setAlignment (Qt::AlignHCenter);
  qso_count.setMinimumSize (QSize {30, 18});
  qso_count.setFrameStyle (QFrame::Panel | QFrame::Sunken);
  statusBar()->addWidget (&qso_count);

  labAz.setAlignment (Qt::AlignHCenter);
  labAz.setMinimumSize (QSize {30, 18});
    ...
    if(line_read.indexOf("<DecodeFinished>") >= 0) {
      ...
      if (line_read.left(ntime) != m_tBlankLine) {
          ...
          if (m_config.insert_blank ()
            ...
            // Z
            ui->tableBand->addSeparator(band);
            ui->tableRx->addSeparator(band);
            if (m_unfilteredView && m_unfilteredView->isVisible()) {
      ...
      if(!bAvgMsg) {
        ...
        if (m_unfilteredView && m_unfilteredView->isVisible()) {
          ...
          if (!isFiltered || for_us)
          {
              ...
              if (m_config.showState())  {
                  state = stateLookup(deCall);
              }

                ui->tableBand->addRow(decodedtext1, m_logBook, m_currentBand, m_mode, (ui->dxCallEntry->text() == deCall && m_config.highlightDX()), ui->cbCQonly->isChecked(),
                                      ui->cbCQonlyIncl73->isChecked(), isFiltered, state);
...
// multi-station special events.
      ...
      if (bDisplayRight) {
        ...
        if(!m_bBestSPArmed or m_mode!="FT4") {
            // Z
            ui->tableRx->addRow(decodedtext0, m_logBook, m_currentBand, m_mode, false, false, false, isFiltered);

           // addRow(decodedtext1, m_logBook, m_currentBand, m_mode, (ui->dxCallEntry->text() == deCall && m_config.highlightDX()), ui->cbCQonly->isChecked(),
                                                  //ui->cbCQonlyIncl73->isChecked(), isFiltered, state);
                    //addRow(decodedtext0, m_logBook, m_currentBand, m_mode);
        ...
        // Z
      if (!isFiltered || !m_config.udpFiltering())
      postDecode (true, decodedtext.string ());
...
//### I think this is where we are preventing Hounds from spotting Fox ###
	  // Z
	  if (!(isFiltered && (ui->cbAutoCQ->isChecked() || ui->cbAutoCall->isChecked()) && m_config.autoCQfiltering()))
  ...
  if (message_words.size () > 3 && (message.isStandardMessage() || (is_73 or is_OK))) {
    ...
    if (m_auto
      ...
      // Z
               ...
               && (!m_transmitting || hiscall == m_hisCall || m_ntx == 6 || m_lastCall == hiscall)
               )
    {
        log("Processing tail-end response");
        processMessage (message);
...
//----------------------------------------------------------------------
    ...
    if(m_restart) {
      ...
      if (m_config.TX_messages () and m_mode!="Echo") {
        ...
        log("TX message: " + m_currentMessage.trimmed());
        ui->tableRx->addTxRow(m_currentMessage.trimmed(), ui->TxFreqSpinBox->value());
        }
    }
        ...
        if (m_config.TX_messages () && !m_tune && SpecOp::FOX!=m_specOp)
          {
            ...
            log("TX message: " + current_message.trimmed());
            ui->tableRx->addTxRow(m_currentMessage.trimmed(), ui->TxFreqSpinBox->value());
          }
      }
...
//    qDebug() << "AAA" << nsec << m_bFastMode << m_bFast9;
	...
	*/
    ...
    astroUpdate ();
    // Z
    QString green = "QProgressBar::chunk {background: #00aa00; border-bottom-right-radius: 5px;border-bottom-left-radius: 5px;border: .px solid black; text-align: center}";
    QString red = "QProgressBar::chunk {background: #ff0000; border-bottom-right-radius: 5px;border-bottom-left-radius: 5px;border: .px solid black; text-align: center}";

    if(m_transmitting) {
        // Z
        progressBar.setStyleSheet(red);
      char s[42];
      if(SpecOp::FOX==m_specOp and ui->tabWidget->currentIndex()==1) {
      ...
      if(m_tune) {
        ...
        // Z
        progressBar.setStyleSheet(green);
      if (!m_tx_watchdog) {
...
void MainWindow::on_txrb6_toggled(bool status)
{
  if (status) {
    // Z
    if(ui->cbAutoCQ->isChecked()) auto_tx_mode(true);
...
void MainWindow::processMessage (DecodedText const& message, Qt::KeyboardModifiers modifiers)
{
  ...
  // Z
  auto auto_seq = ui->cbAutoSeq->isChecked ();
...
{
	 // Z
    log("on_logQSOButton_clicked!");
    if (!m_hisCall.size () || m_lastCall == m_hisCall) {
        log("on_logQSOButton_clicked: m_hisCall is empty, or callsign already logged. Exiting.");
        clearDX();
        m_inQSOwith="";
        return;
    }
  ...
  if(grid=="....") grid="";
  // Z
  if (grid.length() == 0 || (ui->ci_grid->text().length() > grid.length() && ui->ci_grid->text().startsWith(grid)))  grid = ui->ci_grid->text();
...
void MainWindow::on_bandComboBox_activated (int index)
{
  ...
  // Z
  if (m_config.clearRX()) clearRXWindows();
}
...
void MainWindow::on_stopTxButton_clicked()                    //Stop Tx
{
  ...
  // Z
  if (!m_tune) ui->autoButton->setChecked (false);
  if (!m_tune) on_autoButton_clicked (false);
...
void MainWindow::replyToCQ (QTime time, qint32 snr, float delta_time, quint32 delta_frequency
                            ...
                            , bool /*low_confidence*/, __attribute__ ((unused)) quint8 modifiers)
{
    // Z
    tableBandItemClicked(message_text);
  ...
  auto message_line = format_string
    ...
    .arg (mode, -2)
    .arg (text);
    tableBandItemClicked(message_line);
...
void MainWindow::hound_reply ()
{
  if (!m_tune) {
    ...
    // Z
    if (!m_config.noFoxQSY()) ui->TxFreqSpinBox->setValue (m_nFoxFreq);
  }
}
...
{
  // Z
  if(m_config.disableWriteALL()) return;
...
void MainWindow::on_cbAutoCall_toggled(bool b)
{
    ...
    if (b) {
        ...
        ui->cb_callB4onBand->setChecked(true);
...
bool MainWindow::callsignFiltered(DecodedText dt)
{
    ...
    log("callsignFiltered: ENTRY");

    auto const& message_words = dt.messageWords ();
    if (message_words.size() == 0) return false;
    log("message:" + dt.string());
    dt.deCallAndGrid (/*out*/ dxCall, dxGrid);
    log("dxCall: " + dxCall);
    int nmod = fmod(double(dt.timeInSeconds()),2.0*m_TRperiod);
    auto m_txFirst=(nmod!=0);

    if (!dxCall.contains(QRegExp("[0-9]")) || dxCall.length() < 3) {
        log("callsignFiltered: Invalid callsign. Skipping.");
        return true;
    }

    if (dxCall.endsWith("/R")) {
        log("callsignFiltered: False decode (ends with /R). Skipping.");
        return true;
    }

    bool is_73 = (message_words.size() >= 5 && (message_words.contains("73") || message_words.contains("RR73")));
    bool is_CQ = message_words.filter (QRegularExpression {"^(CQ|CQ\\s\\w+)$"}).size();

    // Auto call next
    if (ui->cb_autoCallNext->isChecked() && dxCall == ui->dxCallEntry->text() ) {
          if (is_CQ || is_73 || SpecOp::HOUND == m_config.special_op_id()) {

              if (m_TxFirstLock && (ui->txFirstCheckBox->isChecked() != m_txFirst)) {
                log("callsignFiltered: TX First Lock. Pounce cancelled.");
                return false;
              } else {
                  ...
                  m_prioGrid  = dxGrid;
                  log("callsignFiltered: Pounce mode");
                  return false;
              }
          }
     }
    ...
    if ( ui->cb_f_LOTW->isChecked() && !m_config.lotw_users ().user (dxCall)) {
        log("callsignFiltered: User not in LOTW");
        return true;
    }

    // Ignored stations filter
    QStringList ignoredStations = ui->pte_IgnoredStations->toPlainText().split(QRegExp("[\r\n]"),SkipEmptyParts);
    if (ignoredStations.contains(dxCall)) {
        log("callsignFiltered: Station is in the ignore list");
        return true;
    }

    if (m_lastCall == dxCall || m_hisCall == dxCall) {
        log("Current or last QSO partner.");
        return false;
    }
    ...
    if (ui->sbMindB->value() > -30 && dbM.toInt() < ui->sbMindB->value()) {
        log("callsignFiltered: Station signal strength under threshold: " + dbM);
        return true;
    }
    ...
    log("callsignFiltered: Continent filtering...");
    ...
    log("callsignFiltered: CQ Target filtering...");
    ...
    if( message_words.size() > 3 && ( ui->cb_ignoreCQTarget->currentIndex() > 0 || ui->cb_filter_CQDX_Continent->currentIndex() > 0) && message_words[2].startsWith("CQ")) {
        ...
        QString w1 = message_words[2];
        QString w2 = message_words[3];

        if (w2.length()<=4 && !w2.contains(QRegExp("[0-9]"))) {
            CQTarget = w2;
        } else {
            CQTarget = w1.mid(w1.indexOf(" ")+1, -1);
        }

        log("++ CQ Target: " + CQTarget);
        if ((CQTarget == "CQ" || CQTarget == "DX") && ui->cb_ignoreCQTarget->currentIndex() == 3) return true;
            ...
            if (CQTarget != "CQ")
            {
                ...
                if (CQTarget == "DX" && ui->cb_filter_CQDX_Continent->currentIndex() > 0 ) {
                    log("callsignFiltered: CQ DX filtering...");
                    int idx = ui->cb_filter_CQDX_Continent->currentIndex();
                    switch (idx) {
                        case 1:
                            if (continent == "AF") return true;
                            break;

                        case 2:
                            if (continent == "NA") return true;
                            break;

                        case 3:
                            if (continent == "SA") return true;
                            break;

                        case 4:
                            if (continent == "OC") return true;
                            break;
                        ...
                        case 6:
                            if (continent == "AS") return true;
                            break;

                        case 7:
                            if (continent == "AN") return true;
                            break;
    ...
    // New / New on band filter
    if (ui->cb_callB4->isChecked() || ui->cb_callB4onBand->isChecked() || ui->cb_countryB4->isChecked() || ui->cb_countryB4onBand->isChecked() ||
            ui->cb_gridB4->isChecked() || ui->cb_gridB4onBand->isChecked() || ui->cb_continentB4->isChecked() || ui->cb_continentB4onBand->isChecked() ||
            ui->cb_CQZoneB4->isChecked() || ui->cb_CQZoneB4onBand->isChecked() || ui->cb_ITUZoneB4->isChecked() || ui->cb_ITUZoneB4onBand->isChecked()) {

        log("callsignFiltered: New/New on band filtering...");
        ...
        if (ui->cb_countryB4onBand->isChecked() && countryB4onBand) return true;
        if (ui->cb_gridB4->isChecked() && gridB4) return true;
        if (ui->cb_gridB4onBand->isChecked() && gridB4onBand) return true;
        if (ui->cb_continentB4->isChecked() && continentB4) return true;
        if (ui->cb_continentB4onBand->isChecked() && continentB4onBand) return true;
        if (ui->cb_CQZoneB4->isChecked() && CQZoneB4) return true;
        if (ui->cb_CQZoneB4onBand->isChecked() && CQZoneB4onBand) return true;
        if (ui->cb_ITUZoneB4->isChecked() && ITUZoneB4) return true;
        if (ui->cb_ITUZoneB4onBand->isChecked() && ITUZoneB4onBand) return true;

    }

    // Prefix filter
    if (ui->cb_prefixFilter->currentIndex() > 0) {
        log("callsignFiltered: Prefix filtering...");
        QString filterText = ui->pte_prefixFilter->toPlainText();
        QStringList filterLines = filterText.split(QRegExp("[\r\n]"),SkipEmptyParts);
        int filterIndex = filterLines.indexOf(QRegExp("^" + m_currentBand +":.*"));
    ...
    if (ui->cb_stateFilter->currentIndex() > 0) {
        ...
        if  (country == "United States")  {
            ...
            log("callsignFiltered: US State filtering: " + state);

            QString filterText = ui->pte_stateFilter->toPlainText();
            QStringList filterLines = filterText.split(QRegExp("[\r\n]"),SkipEmptyParts);
            int filterIndex = filterLines.indexOf(QRegExp("^" + m_currentBand +":.*"));
            QStringList filterPrefixes;
            if (filterIndex > -1) {
    ...
    // Alerts
          if ((is_CQ || (ui->cbCQonlyIncl73->isChecked() && is_73)) &&
           (ui->cb_callB4_alert->isChecked() || ui->cb_callB4onBand_alert->isChecked() || ui->cb_countryB4_alert->isChecked() || ui->cb_countryB4onBand_alert->isChecked() ||
                  ...
                  ui->le_CustomAlerts->text().length() > 0)) {
              ...
              if (customAlert ||
               (ui->cb_callB4_alert->isChecked() && !callB4) ||
               (ui->cb_callB4onBand_alert->isChecked() && !callB4onBand)  ||
               (ui->cb_countryB4_alert->isChecked() && !countryB4)  ||
               (ui->cb_countryB4onBand_alert->isChecked() && !countryB4onBand)  ||
               (ui->cb_gridB4_alert->isChecked() && !gridB4)  ||
               (ui->cb_gridB4onBand_alert->isChecked() && !gridB4onBand)  ||
               (ui->cb_continentB4_alert->isChecked() && !continentB4)  ||
               (ui->cb_continentB4onBand_alert->isChecked() && !continentB4onBand)  ||
               (ui->cb_CQZoneB4_alert->isChecked() && !CQZoneB4)  ||
               (ui->cb_CQZoneB4onBand_alert->isChecked() && !CQZoneB4onBand)  ||
               (ui->cb_ITUZoneB4_alert->isChecked() && !ITUZoneB4)  ||
               (ui->cb_ITUZoneB4onBand_alert->isChecked() && !ITUZoneB4onBand)) {
                  if (!m_beeped) {
                      QApplication::beep();
                      ui->pte_alerts->appendPlainText( "[" + m_currentBand +"] " + QDateTime::currentDateTimeUtc().toString());
                      if (m_config.alertCmdLine().length() > 0) {
                            QString cmd = m_config.alertCmdLine();
                            cmd = cmd.replace("%band%", m_currentBand);
                            cmd = cmd.replace("%call%", dxCall);
                            cmd = cmd.replace("%dxcc%", looked_up.entity_name);
                            cmd = cmd.replace("%datetime%", QDateTime::currentDateTimeUtc().toString());
                            execCmd(cmd);
                      }

                  }
                  ui->pte_alerts->appendPlainText(dxCall.leftJustified(12, ' ') + looked_up.entity_name);
                  m_beeped = true;
              }
          }

    log("Call not filtered: " + dxCall);

    if ( !is_CQ && !(ui->cbCQonlyIncl73->isChecked() && is_73) ) {
        log("Not CQ/73. Exiting.");
        return false;
    }


    if (m_TxFirstLock && (ui->txFirstCheckBox->isChecked() != m_txFirst)) {
        log("callsignFiltered: TX First Lock. Exiting.");
        return false;
    }

    if (ui->cb_autoCallNext->isChecked()) {
        log("callsignFiltered: Pounce mode. Exiting.");
        return false;
    }

    bool prio = false;
    if (ui->respondComboBox->currentIndex() == 2) {
    ...
    } else if (ui->respondComboBox->currentIndex() == 3) {
        bool convOK;
    ...
    log("callsignFiltered: EXIT");
    return false;
}

QString MainWindow::getClickedMessage() {
    QString message = "";
    int row;

    if (currentPaneId == 0) {
        row = ui->tableBand->currentRow();
        if (row > -1) message = ui->tableBand->item(row, 0)->toolTip();
    } else {
        row = ui->tableRx->currentRow();
        if (row > -1) message = ui->tableRx->item(row, 0)->toolTip();
    }
...
void MainWindow::on_actionIgnore_station_triggered() {
    ...
    QString dxCall;
    QString dxGrid;
    QString message = getClickedMessage();
...
void MainWindow::on_actionCall_next_triggered() {
    QString dxCall;
    QString dxGrid;
    QString message = getClickedMessage();
    if (message.isEmpty()) return;
    ...
    int frequency = dt.frequencyOffset();
...
void MainWindow::on_cbMini_toggled(bool b) {
        ui->signal_meter_widget->setVisible(!b);
        ui->QSO_controls_widget->setVisible(!b);
        ...
        ui->genStdMsgsPushButton->setVisible(!b);
        ui->lookupButton->setVisible(!b);
        ui->addButton->setVisible(!b);
}


void MainWindow::on_cbCQonlyIncl73_toggled(bool b) {
    if (b) {
        ui->cbHoldTxFreq->setChecked(true);
    }
}
...
void MainWindow::on_actionSet_Rx_Freq_triggered() {
...
void MainWindow::on_actionQRZ_Lookup_triggered() {
    QString dxCall;
    QString dxGrid;
...
{
    if (!ui->cbDockWF->isChecked()) return;
    ...
    int x = m_lastX - this->pos().x();
    int y = m_lastY - this->pos().y();
    m_lastX = this->pos().x();
    m_lastY = this->pos().y();
    ...
    if (m_unfilteredView &&  m_unfilteredView->isVisible()) {
        ...
        m_unfilteredView->move(rx-x,ry-y);
    }

    if (m_pskReporterView &&  m_pskReporterView->isVisible()) {
        int rx = m_pskReporterView->pos().x();
        int ry = m_pskReporterView->pos().y();
        m_pskReporterView->move(rx-x,ry-y);
    }
}
...
void MainWindow::on_actionCopy_triggered() {
    QApplication::clipboard()->setText(getClickedMessage());
 }

void MainWindow::on_txrb6_doubleClicked ()
{
    ui->dxCallEntry->clear();
    ui->dxGridEntry->clear();
    genStdMsgs ("0", true);
...
void MainWindow::leftClickHandler(Qt::KeyboardModifiers modifiers) {
...
void MainWindow::dxLookup(QString dxCall, QString dxGrid) {
    ...
    ui->ci_dxcc->setText(looked_up.entity_name);
    ui->ci_dxcc->setCursorPosition(0);
    ...
    if (!continentB4) ui->ci_continent->setFont(bold);
    if (!countryB4) ui->ci_dxcc->setFont(bold);
    if (!CQZoneB4) ui->ci_cqzone->setFont(bold);
...
void MainWindow::qrzVisible(bool b) {
    ui->l_q_name->setVisible(b);
    ui->l_q_email->setVisible(b);
    ui->l_q_state->setVisible(b);
    ui->l_q_zipcode->setVisible(b);
    ui->q_name->setVisible(b);
    ui->q_email->setVisible(b);
    ui->q_state->setVisible(b);
    ui->q_zipcode->setVisible(b);
    ui->q_addr1->setVisible(b);
    ui->q_addr2->setVisible(b);
    ui->l_q_addr1->setVisible(b);
    ui->l_q_addr2->setVisible(b);
    ui->q_button->setVisible(b);
}

void MainWindow::qrzInit() {
    log("ENTRY: qrzInit");
    QUrlQuery query;
...
void MainWindow::tableInit() {
    ui->tableBand->set_configuration (&m_config);
    ui->tableRx->set_configuration (&m_config);
    ui->tableRx->init(1);
    ui->tableBand->init(0);
    ui->tableRx->setRowCount(0);
    ui->tableBand->setRowCount(0);
...
void MainWindow::qrzSetSessionKey(QNetworkReply *r) {
    log("ENTRY: qrzSetSessionKey");
    disconnect(networkManager, SIGNAL(finished(QNetworkReply*)), this, SLOT(qrzSetSessionKey(QNetworkReply*)));
    qrzSessionKey = "";
    QString error;
    if(r->error() == QNetworkReply::NoError) {
        ...
        while(!reader.atEnd() && !reader.hasError()) {
            if(reader.readNext() == QXmlStreamReader::StartElement) {
                if (reader.name() == "Key") {
                    qrzSessionKey = reader.readElementText();
                }
    ...
    if (!error.isNull()) {
        ...
        if (!qrzPendingLookupCall.isEmpty()) {
            qrzLookup(qrzPendingLookupCall);
        }
    }

    r->deleteLater();
    log("EXIT: qrzSetSessionKey");
}

void MainWindow::qrzLookup(QString dxCall) {
    if (qrzSessionKey.isNull() || qrzSessionKey.isEmpty() ) return;

    // TODO: if (dxCall.contains("/")
    qrzPendingLookupCall = dxCall;
...
void MainWindow::qrzResponseHandler(QNetworkReply * r) {
    ...
    if(r->error() == QNetworkReply::NoError) {
        ...
        while(!reader.atEnd() && !reader.hasError()) {
            if(reader.readNext() == QXmlStreamReader::StartElement) {
                if ( reader.name() == "fname") {
                    ui->q_name->setText(reader.readElementText());
                    continue;
                }

                if ( reader.name() == "name") {
                    ui->q_name->insert(" " + reader.readElementText());
                    ui->q_name->setCursorPosition(0);
                    continue;
                }

                if ( reader.name() == "addr1") {
                    ui->q_addr1->setText(reader.readElementText());
                    ui->q_addr1->setCursorPosition(0);
                    continue;
                }

                if ( reader.name() == "addr2") {
                    ui->q_addr2->setText(reader.readElementText());
                    ui->q_addr2->setCursorPosition(0);
                    continue;
                }
                ...
                if ( reader.name() == "state") {
                    ui->q_state->setText(reader.readElementText());
                    ui->q_state->setCursorPosition(0);
                    continue;
                }

                if ( reader.name() == "zip") {
                    ui->q_zipcode->setText(reader.readElementText());
                    ui->q_zipcode->setCursorPosition(0);
                    continue;
                }
        ...
        if (error == "Invalid session key" || error == "Session Timeout") {
            ...
            if (ui->ci_grid->text().length() > ui->dxGridEntry->text().length() && qrzPendingLookupCall == ui->dxCallEntry->text()
                    && ui->ci_grid->text().toUpper().startsWith(ui->dxGridEntry->text().toUpper()))
                ui->dxGridEntry->setText(ui->ci_grid->text());
...
void MainWindow::log(QString s) {
    if (!ui->actionWSJT_Z_Debug->isChecked()) return ;
    QString zdebug = "ZDebug.log";

    if (m_config.dbgScreen() || m_config.dbgBoth()) ui->decodedTextBrowser2->insertText(s, "#ff5050");

    if (m_config.dbgFile() || m_config.dbgBoth()) {
        QFile f{m_config.writeable_data_dir().absoluteFilePath(zdebug)};
        if (f.open(QIODevice::WriteOnly | QIODevice::Text | QIODevice::Append)) {
            QTextStream out(&f);
            out << s
#if QT_VERSION >= QT_VERSION_CHECK (5, 15, 0)
...
void MainWindow::clearCallInfo() {
    ...
    QFont bold = ui->ci_continent->font();
    bold.setWeight(QFont::Normal);

    ui->ci_continent->setFont(bold);
    ui->ci_dxcc->setFont(bold);
    ui->ci_cqzone->setFont(bold);
    ui->ci_ituzone->setFont(bold);
    ui->ci_dxcall->setFont(bold);
}

void MainWindow::on_cb_filtering_toggled(bool b) {
    if (b) {
       ui->cb_filtering->setStyleSheet("background-color: #00aa00;");
    } else {
...
void MainWindow::on_cb_specialMode_currentIndexChanged (int index)
{
    ...
    case 0:
        m_config.setSpecial_None();
        ui->cb_specialMode->setStyleSheet("");
        break;
    case 1:
        m_config.setSpecial_Fox();
        ui->cb_specialMode->setStyleSheet("background-color: #ff0000;");
        break;
    case 2:
        m_config.setSpecial_Hound();
        ui->cb_specialMode->setStyleSheet("background-color: #ff0000;");
...
void MainWindow::on_ci_pb_lookup_clicked() {
    dxLookup(ui->ci_dxcall->text(), "");
}

void MainWindow::ci_gridLookup() {
    QString dxGrid = ui->ci_grid->text();
    if (dxGrid.length() < 4) return;
    ...
    ui->ci_distance->setText(t);
    ui->ci_bearing->setText(QString::number(nAz));
}
...
void MainWindow::ZMessage ()
{
    ...
    QString message = "Please visit our <a href='https://groups.io/g/WSJT-Z/topics'>groups.io</a> forum if you need help with Z! <br /><br />"
                        ...
                        "<a href='https://sourceforge.net/projects/wsjt-z/files/Documentation/WSJT-Z%20User%20Manual.pdf/download'>WSJT-Z Documentation</a> ";

    MessageBox::information_message(this, message);
}

void MainWindow::on_actionAbout_WSJT_Z_triggered ()
{
    ZMessage ();
}
...
void MainWindow::ZProcess ()
{
    ...
    {
        m_priorityCall = QString();
        log("ZProcess: EXIT (Transmitting)");
        return;
    }

    if (m_config.ignoreListReset() != 0 && m_ignoreListReset.secsTo(QDateTime::currentDateTime()) / 60 >= m_config.ignoreListReset()) {
        log("ZProcess: Clearing ignore list");
        on_btn_clearIgnore_clicked();
    }

    if (ui->cbAutoCall->isChecked() && ui->txrb6->isChecked()) {
        clearDX();
    }

    log("ZProcess: autoButton: " + QString::number(ui->autoButton->isChecked()));
    log("ZProcess: cbAutoCall: " + QString::number(ui->cbAutoCall->isChecked()));
    log("ZProcess: cbnAutoCQ: " + QString::number(ui->cbAutoCQ->isChecked()));
    log("ZProcess: m_priorityCall: " + m_priorityCall);
    log("ZProcess: dxCallEntry: " + ui->dxCallEntry->text());
    log("ZProcess: m_hiscall: " + m_hisCall);
    log("ZProcess: m_lastcall: " + m_lastCall);
    ...
    if (m_QSOProgress == CALLING) {
        log("ZProcess: m_QSOProgress = CALLING");
                ...
                if (ui->cbAutoCall->isChecked()) {
                    ...
                    if (l > 1){
                        ...
                        if (ui->cb_autoModeSwitch->isChecked()) {
                            ...
                            toggleBands();
                            log("ZProcess: Switched to AutoCQ");
                        } else {
                            toggleBands();
                        }
                    }
                    ...
                    if (l > 1){                      
                        if (m_autoTXFreq) {
                          ...
                          if (ui->cb_autoModeSwitch->isChecked()) {
                              ...
                              m_autoModeSwitch = false;
                              log("ZProcess: Switched to AutoCall");
                          } else {
                              toggleBands();
                          }
                    }
    ...
    int numVectors = 3;
    //logSlots();
    if (busySlots.size() >= numVectors) busySlots.removeLast();
...
void MainWindow::on_pb_WDReset_clicked() {
    tx_watchdog(false);
}
...
void MainWindow::on_actionDark_mode_triggered() {
    if (ui->actionDark_mode->isChecked()) {
        ...
        log("Stylesheet: LIGHT");
        qApp->setStyleSheet("");
        labAz.setStyleSheet ("QLabel{background-color: #00bbbb; padding-left: 10px; padding-right: 10px}");
...
QString MainWindow::stateLookup(QString callsign) {
    ...
    path = m_config.data_dir ().absoluteFilePath (file_name);
    ...
    QString state =  "";
    QSqlQuery q(db);
    q.prepare("SELECT state FROM USState WHERE call = (:call)");
    q.bindValue(":call", callsign);
    if (q.exec())
    {
       if (q.next())
       {
...
void MainWindow::on_actionUnfiltered_View_triggered() {
    if (m_unfilteredView && m_unfilteredView->isVisible()) {
        ...
        m_unfilteredView.reset (new UnfilteredView {});
        connect (this, &MainWindow::finished, m_unfilteredView.data (), &UnfilteredView::close);
        m_unfilteredView->restoreGeometry(m_unfilteredViewGeometry);
        m_unfilteredView->showNormal ();
        m_unfilteredView->setFont(m_config.decoded_text_font ());
        m_unfilteredView->raise ();
        m_unfilteredView->activateWindow ();
    }
}

QString MainWindow::leftJustifyAppendage (QString message, QString appendage)
{
  if (appendage.size ())
    {
...
void MainWindow::clearRXWindows() {
    ui->decodedTextBrowser->erase();
    ui->decodedTextBrowser2->erase();
    ui->tableRx->setRowCount(0);
    ui->tableBand->setRowCount(0);
...
void MainWindow::updateQsoCounter(bool increment) {
    ...
    QString qc = "%1 New / %2 Total";
    qso_count.setText(qc.arg(QString::number(qso_new), QString::number(qso_total)));
}
...
void MainWindow::on_txFirstCheckBox_toggled() {
    ...
    if((ms-m_msTxFirst)<300) {
        if  (m_TxFirstLock) {
            ...
            m_TxFirstLock =  true;
            ui->txFirstCheckBox->setStyleSheet("background-color: #ff0000; color: #ffffff;");
        }
    }
...
void MainWindow::pskTableClicked(QString callsign, QString band) {
    unsigned long long f = band.toULongLong();
    Radio::Frequency freq {f};
...
void MainWindow::logSlots() {
    log("SLOTS========================");
    for (int x = 0; x < busySlots.size(); x++) {
        QVector<int> slotsV = busySlots.at(x);
        for (int y = 0; y < slotsV.size(); y++) {
            log("slot["+QString::number(x)+"] pos["+QString::number(y)+"] " + QString::number(slotsV.at(y)));
        }
    }
...
bool MainWindow::isSlotFree(int f) {
    ...
    for (int x = 0; x < busySlots.size(); x++) {
        QVector<int> slotsV = busySlots.at(x);
        for (int y = 0; y < slotsV.size(); y++) {
            int slotFreq = slotsV.at(y);
            if (!(slotFreq > f+width || slotFreq < f-width)) {
                return false;
            }
        }
    }
...
void MainWindow::execCmd(QString cmd) {
    log("execCMD: " + cmd);
    QString program = cmd.left(cmd.indexOf(" "));
    cmd.remove(0, cmd.indexOf(" ")+1);
    QProcess::startDetached(program, QStringList() << cmd);
}


void MainWindow::tableBandItemClicked(QString message) {

    /*QString message;
    if (id == 0)
        message = ui->tableBand->item(x, 1)->toolTip();
    else
        message = ui->tableRx->item(x, 1)->toolTip();*/
...
void MainWindow::ctrlClicked(QString message) {
    DecodedText decodedtext(message);
    QString call,grid;
    decodedtext.deCallAndGrid(call,grid);
    dxLookup(call, grid);
}

void MainWindow::clicked(int paneId) {
